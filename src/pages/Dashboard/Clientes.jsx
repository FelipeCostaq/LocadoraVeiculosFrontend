...
const Clientes = () => {
  ...
  const handleOpenModal = (cliente = null) => {
    if (cliente) {
      ...
      const formattedDate = cliente.dataNasc ? cliente.dataNasc.split('T')[0] : '';
      setFormData({ ...cliente, dataNasc: formattedDate });
    } else {
      setCurrentCliente(null);
      setFormData({ nome: '', cpf: '', email: '', telefone: '', dataNasc: '', endereco: '', ativo: true });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Erro ao salvar cliente. Verifique se o CPF é único e os dados estão corretos.');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Gerencie o cadastro de seus clientes.</p>
        </div>
        <button 
                  ...
                  <tr key={cliente.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground flex items-center gap-1">
                            <MapPin size={12} /> {cliente.endereco || 'Endereço não informado'}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm font-mono">{cliente.cpf}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${
                        cliente.ativo ? 'bg-green-500/10 text-green-500' : 'bg-destructive/10 text-destructive'
                      }`}>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="bg-card border border-border w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border">
              <h3 className="text-xl font-bold">{currentCliente ? 'Editar Cliente' : 'Novo Cliente'}</h3>
