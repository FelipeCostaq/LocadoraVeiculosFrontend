...
import { Car, Users, CalendarDays, Layers, TrendingUp, Loader2 } from 'lucide-react';
...
const DashboardHome = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setStats({
        veiculos: resVei.data.length,
        clientes: resCli.data.filter(c => c.ativo).length,
        locacoes: resLoc.data.filter(l => l.status === 0).length, // Apenas em andamento
  ...
  const statCards = [
    { label: 'Veículos na Frota', value: stats.veiculos, icon: Car, color: 'text-primary', path: '/dashboard/veiculos' },
    { label: 'Clientes Ativos', value: stats.clientes, icon: Users, color: 'text-blue-500', path: '/dashboard/clientes' },
    { label: 'Locações Ativas', value: stats.locacoes, icon: CalendarDays, color: 'text-green-500', path: '/dashboard/locacoes' },
    { label: 'Categorias', value: stats.categorias, icon: Layers, color: 'text-purple-500', path: '/dashboard/categorias' },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <header>
        ...
        <p className="text-muted-foreground">Aqui está o estado real da sua locadora agora.</p>
      </header>
        ...
        {statCards.map((stat) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <TrendingUp size={16} className="text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => navigate('/dashboard/locacoes')} className="text-primary text-sm font-bold hover:underline">Ver tudo</button>
          </div>
          <div className="space-y-4">
            {ultimasLocacoes.length > 0 ? (
              ultimasLocacoes.map((loc) => (
                <div key={loc.id} className="flex items-center justify-between p-4 bg-muted/30 rounded-xl border border-border/50">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center text-primary">
                      <Car size={20} />
                    </div>
                    <div>
                      <p className="font-bold text-sm uppercase">{loc.placaVeiculo}</p>
                      <p className="text-xs text-muted-foreground">Cliente ID: {loc.clienteId.substring(0, 8)}...</p>
                    </div>
                  </div>
                  <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${
                    loc.status === 1 ? 'bg-muted text-muted-foreground' : (loc.status === 0 ? 'bg-green-500/10 text-green-500' : 'bg-destructive/10 text-destructive')
                  }`}>
                    {loc.status === 1 ? 'CONCLUÍDA' : (loc.status === 0 ? 'EM ANDAMENTO' : 'CANCELADA')}
                  </span>
                </div>
              ))
            ) : (
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              <CalendarDays size={32} className="text-green-500 group-hover:scale-110 transition-transform" />
